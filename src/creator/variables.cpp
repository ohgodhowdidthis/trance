...
, _session(session)
{
  ...
  auto left =
      new wxStaticBoxSizer{wxVERTICAL, left_panel, "Variable values"};
  auto right_panel = new wxPanel{bottom_splitter, wxID_ANY};
  auto right =
      new wxStaticBoxSizer{wxVERTICAL, right_panel, "Variable information"};
  ...
  _item_list = new ItemList<trance_pb::Variable>{
      ...
      std::bind(&CreatorFrame::VariableCreated, &_creator_frame,
                std::placeholders::_1),
      std::bind(&CreatorFrame::VariableDeleted, &_creator_frame,
                std::placeholders::_1),
      std::bind(&CreatorFrame::VariableRenamed, &_creator_frame,
                std::placeholders::_1, std::placeholders::_2), true};

  _value_list = new wxListCtrl{
      left_panel, wxID_ANY, wxDefaultPosition, wxDefaultSize,
      wxLC_REPORT | wxLC_NO_HEADER | wxLC_SINGLE_SEL | wxLC_EDIT_LABELS};
  _value_list->InsertColumn(0, "Value", wxLIST_FORMAT_LEFT,
                            wxLIST_AUTOSIZE_USEHEADER);
  _value_list->SetToolTip("Potential values that this variable can be set to "
  ...
  right->Add(label, 0, wxALL, DEFAULT_BORDER);
  _description = new wxTextCtrl{right_panel, wxID_ANY, "", wxDefaultPosition,
  ...
  _value_list->Bind(wxEVT_SIZE, [&](wxSizeEvent&)
  {
    _value_list->SetColumnWidth(0, wxLIST_AUTOSIZE_USEHEADER);
  }, wxID_ANY);

  _value_list->Bind(wxEVT_LIST_ITEM_SELECTED, [&](wxListEvent& e)
  {
    _current_value = _value_list->GetItemText(e.GetIndex());
  }, wxID_ANY);

  _value_list->Bind(wxEVT_LIST_END_LABEL_EDIT, [&](wxListEvent& e)
  {
    ...
    _creator_frame.VariableValueRenamed(
        _item_selected, old_name, new_name);
    RefreshData();
  }, wxID_ANY);

  left_panel->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [&](wxCommandEvent&)
  {
    static const std::string new_name = "New value";
    auto name = new_name;
    int number = 0;
    auto& data = (*_session.mutable_variable_map())[_item_selected];
    while (std::find(data.value().begin(), data.value().end(), name) !=
           data.value().end()) {
  ...
  left_panel->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [&](wxCommandEvent&)
  {
    ...
    if (removed >= 0) {
      for (auto it = data.mutable_value()->begin();
           it != data.mutable_value()->end();) {
    ...
    if (removed >= 0) {
      ...
      _value_list->SetItemState(
          index, wxLIST_STATE_SELECTED, wxLIST_STATE_SELECTED);
    }
    _creator_frame.MakeDirty(true);
  }, ID_DELETE);
  ...
  _default_value->Bind(wxEVT_CHOICE, [&](wxCommandEvent&) {
    ...
    it->second.set_default_value(
        _default_value->GetString(_default_value->GetSelection()));
    _creator_frame.MakeDirty(true);
  });
...
void VariablePage::RefreshOurData()
{
  ...
  if (it != _session.variable_map().end()) {
    std::vector<std::string> values{it->second.value().begin(),
                                    it->second.value().end()};
